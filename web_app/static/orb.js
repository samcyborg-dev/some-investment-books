...
function strategy2Status(text) {
  const value = String(text || "").toUpperCase();
  const kind = value.includes("PASS") && !value.includes("PENDING")
    ? "model"
    : value.includes("BLOCK") || value.includes("FAIL")
      ? "critical"
      : value.includes("ANECD") || value.includes("REPORT")
        ? "reported"
        : "warning";
  return tag(esc(text), kind);
}
function strategy2External(label, url) {
  return url
    ? `<a class="text-link strategy2-external" href="${esc(url)}" target="_blank" rel="noreferrer">${esc(label)} ${icon("external")}</a>`
    : "";
}
function strategy2RulesTable(s) {
  return `<div class="table-wrap"><table class="data-table strategy2-table"><thead><tr><th>RULE</th><th>FROZEN CANDIDATE</th><th>IMPLEMENTATION NOTE</th></tr></thead><tbody>${s.candidate_rules.map((r) => `<tr><td><strong>${esc(r[0])}</strong></td><td>${esc(r[1])}</td><td>${esc(r[2])}</td></tr>`).join("")}</tbody></table></div>`;
}
function strategy2MetricTable(s) {
  return `<div class="table-wrap"><table class="data-table strategy2-table metric-registry"><thead><tr><th>GROUP</th><th>METRIC</th><th>DEFINITION / FORMULA</th><th>INTERPRETATION</th><th>READINESS</th></tr></thead><tbody>${s.metrics.map((m) => `<tr><td>${tag(esc(m[0]), m[0] === "Risk" ? "warning" : m[0] === "Statistical" ? "reported" : "model")}</td><td><strong>${esc(m[1])}</strong></td><td>${esc(m[2])}</td><td>${esc(m[3])}</td><td>${tag(esc(m[4]), "warning")}<small class="metric-status-note">No value until accepted data and a declared execution model exist.</small></td></tr>`).join("")}</tbody></table></div>`;
}
function strategy2Studies(s) {
  return s.academic_research.map((study) => `<details class="strategy2-study"><summary><span><strong>${esc(study.title)}</strong><small>${esc(study.authors)} · ${esc(study.type)}</small></span>${tag("ACADEMIC", "model")}${icon("chevron")}</summary><div class="strategy2-study-body"><p><strong>Finding</strong>${esc(study.finding)}</p><p><strong>Transfer lesson</strong>${esc(study.lesson)}</p><p><strong>Limitation for this project</strong>${esc(study.limitation)}</p><div class="strategy2-links">${strategy2External("Primary page", study.url)}${strategy2External("Working paper / SSRN", study.ssrn)}${strategy2External("DOI", study.doi)}</div></div></details>`).join("");
}
function strategy2PractitionerTable(s) {
  return `<div class="table-wrap"><table class="data-table strategy2-table practitioner-table"><thead><tr><th>SOURCE / DATE</th><th>CLAIM</th><th>FEEDBACK, FAILURE OR EXECUTION ISSUE</th><th>GRADE</th></tr></thead><tbody>${s.practitioner_scan.map((p) => `<tr><td><strong>${strategy2External(p.source, p.url)}</strong><small>${esc(p.date)}</small></td><td>${esc(p.claim)}</td><td>${esc(p.feedback)}</td><td>${tag(esc(p.evidence_grade), "reported")}</td></tr>`).join("")}</tbody></table></div>`;
}
function strategy2StylesTable(s) {
  return `<div class="table-wrap"><table class="data-table strategy2-table styles-table"><thead><tr><th>STYLE</th><th>MECHANISM / DATA</th><th>STRENGTH</th><th>FAILURE RISK</th><th>FUNDED USE</th><th>PRIVATE CAPITAL USE</th></tr></thead><tbody>${s.styles.map((x) => `<tr><td><strong>${esc(x.name)}</strong></td><td>${esc(x.mechanism)}<small>${esc(x.data)}</small></td><td>${esc(x.strength)}</td><td>${esc(x.risk)}</td><td>${esc(x.funded)}</td><td>${esc(x.private)}</td></tr>`).join("")}</tbody></table></div>`;
}
function renderStrategy2() {
  const s = state.data.strategy2;
  const ev = s.evidence_status;
  return `${pageHead("STRATEGY 02 · CANDIDATE", esc(s.title), "A full research manual for a conditional Z-score mean-reversion hypothesis—without turning untested outputs into performance claims.", `<a class="button" href="/api/orb/download/strategy2-content.json" download>${icon("download")}Content JSON</a>`)}<div class="callout strategy2-callout">${icon("shield")}<div><strong>Candidate only. No ES/MES performance result is being claimed.</strong> The exact rule set remains unvalidated until independent data, causal execution logic, cost reconciliation, out-of-sample testing, and forward observation pass the gates below. This page deliberately separates academic findings, trader reports, project-owned evidence, and illustrative outputs.</div></div><div class="strategy2-status-grid"><article><span>Overall status</span><strong>${esc(ev.overall)}</strong><small>Specification and research preflight</small></article><article><span>Performance</span><strong>${esc(ev.performance)}</strong><small>Net P&amp;L, risk and regime metrics</small></article><article><span>Data</span><strong>${esc(ev.data)}</strong><small>ES and MES must remain independent</small></article><article><span>Internet scan</span><strong>${esc(ev.internet)}</strong><small>${esc(s.as_of)} · no trader claim verified</small></article></div><div class="strategy2-taxonomy"><span>${tag("ACADEMIC", "model")} Published research and papers</span><span>${tag("ANECDOTAL", "reported")} Self-reported trader/public-script feedback</span><span>${tag("PROJECT-OWNED", "critical")} No populated strategy results yet</span><span>${tag("ILLUSTRATIVE", "warning")} Never a substitute for accepted data</span></div><section class="card"><div class="card-head"><div><h2>Manual · how the hypothesis works</h2><p class="card-subtitle">Mean is a conditional reference, not a promise that price must return.</p></div>${tag("RESEARCH MANUAL", "dark-tag")}</div><div class="card-body strategy2-manual"><div class="strategy2-copy-grid"><div><h3>What is statistical mean reversion?</h3><p>${esc(s.manual.what_is_it)}</p></div><div><h3>What it is not</h3><p>${esc(s.manual.what_it_is_not)}</p></div><div><h3>Why this candidate</h3><p>${esc(s.manual.why_this_candidate)}</p></div></div><div class="strategy2-subsection"><h3>Core math and signal interpretation</h3><div class="strategy2-bullet-grid">${s.manual.mathematics.map((x) => `<p>${esc(x)}</p>`).join("")}</div><div class="strategy2-interpretation"><strong>How to read it without overclaiming</strong>${s.manual.interpretation.map((x) => `<p>${esc(x)}</p>`).join("")}</div></div><div class="strategy2-subsection"><h3>Operating workflow</h3><ol>${s.manual.workflow.map((x) => `<li>${esc(x)}</li>`).join("")}</ol></div><div class="strategy2-subsection"><h3>Execution checklist</h3><ol>${s.manual.checklist.map((x) => `<li>${esc(x)}</li>`).join("")}</ol></div><div class="strategy2-subsection"><h3>Execution and risk discipline</h3><ol>${s.manual.execution.map((x) => `<li>${esc(x)}</li>`).join("")}</ol></div><div class="strategy2-subsection"><h3>Known failure modes</h3><div class="strategy2-failure-grid">${s.manual.failure_modes.map((x, i) => `<div><span>${String(i + 1).padStart(2, "0")}</span><p>${esc(x)}</p></div>`).join("")}</div></div></div></section><section class="card"><div class="card-head"><div><h2>Candidate rulebook</h2><p class="card-subtitle">Frozen starting specification · ES and MES are analyzed independently</p></div>${tag("UNVALIDATED", "warning")}</div>${strategy2RulesTable(s)}<div class="card-note">The target priority, same-bar barrier ordering, data source, and acceptable cost model must be frozen before an out-of-sample run. The existing Python implementation is not accepted evidence.</div></section><section class="card"><div class="card-head"><div><h2>Account-use recommendation</h2><p class="card-subtitle">Different objectives require different validation and portfolio designs.</p></div></div><div class="strategy2-recommend-grid">${s.recommended_use.map((x) => `<article class="strategy2-recommend"><div class="strategy2-recommend-top"><h3>${esc(x.account)}</h3>${tag(x.account.includes("Funded") ? "FIRST CANDIDATE" : "SEPARATE BRANCH", x.account.includes("Funded") ? "model" : "reported")}</div><p><strong>Recommendation</strong>${esc(x.recommendation)}</p><p><strong>Why</strong>${esc(x.why)}</p><p><strong>Do not promote yet</strong>${esc(x.not_yet)}</p></article>`).join("")}</div></section><section class="card"><div class="card-head"><div><h2>Mean-reversion styles compared</h2><p class="card-subtitle">A label is not a common strategy: reference, universe, horizon, and execution change the evidence.</p></div></div>${strategy2StylesTable(s)}<div class="card-note">Recommendation: research the single-series candidate first for controlled funded-style paper testing; investigate diversified spread/residual variants separately for private capital. Do not pool the results.</div></section><section class="card"><div class="card-head"><div><h2>Academic research</h2><p class="card-subtitle">Relevant findings and their transfer limits—not validation of our ES/MES rules.</p></div>${tag("ACADEMIC", "model")}</div><div class="strategy2-study-list">${strategy2Studies(s)}</div><div class="card-note">Published papers may report their own historical returns. They are displayed here as academic context only; instruments, samples, costs, and rules differ materially from the candidate.</div></section><section class="card"><div class="card-head"><div><h2>Internet scan · traders and public scripts</h2><p class="card-subtitle">What practitioners claim, what other traders push back on, and where execution breaks.</p></div>${tag("ANECDOTAL", "reported")}</div><div class="callout blue strategy2-inner-callout">${icon("info")}<div>${esc(s.scan_method)}</div></div>${strategy2PractitionerTable(s)}<div class="card-note">These are not independently verified track records. Reddit page fetches were blocked by HTTP 403, so those rows are search-result summaries; no paid script, credential, live order, or vendor result was used.</div></section><section class="card"><div class="card-head"><div><h2>Metric registry · what must be measured</h2><p class="card-subtitle">Every value stays unavailable until the relevant data and execution evidence exist.</p></div>${tag("NOT YET TESTED", "warning")}</div><div class="card-note strategy2-metric-intro">The registry includes return, risk, execution, regime, statistical-validity, funded-account, and private-capital measures. “Required” describes a readiness obligation, not a result.</div><details open class="strategy2-metric-details"><summary>Open the full ${s.metrics.length}-metric registry ${icon("chevron")}</summary>${strategy2MetricTable(s)}</details></section><section class="card"><div class="card-head"><div><h2>Testing and validation gates</h2><p class="card-subtitle">Promotion is blocked by missing data, causal execution, cost, or forward evidence.</p></div>${tag("PROMOTION BLOCKED", "critical")}</div><div class="table-wrap"><table class="data-table strategy2-table"><thead><tr><th>GATE</th><th>ACCEPTANCE TEST</th><th>STATUS</th></tr></thead><tbody>${s.validation_gates.map((g) => `<tr><td><strong>${esc(g[0])}</strong></td><td>${esc(g[1])}</td><td>${strategy2Status(g[2])}</td></tr>`).join("")}</tbody></table></div><div class="strategy2-walkforward"><div><h3>Current position</h3><p>${esc(s.walk_forward.current_position)}</p></div><div><h3>Preferred fold</h3><p>${esc(s.walk_forward.preferred)}</p><p>${esc(s.walk_forward.limited_history)}</p></div><div><h3>Procedure</h3><ol>${s.walk_forward.procedure.map((x) => `<li>${esc(x)}</li>`).join("")}</ol></div></div></section><div class="strategy2-footer-note"><strong>Evidence boundary.</strong> Project-owned Strategy 2 results: none. Illustrative Strategy 2 outputs: none. Academic context and anecdotal feedback are linked above for replication and failure-mode discovery only.</div>`;
}
...
function render() {
  ...
  const routeTitles = { overview: "Opening Range Breakout", lab: "Scenario lab", research: "Research & evidence", trades: "Trade analytics", rules: "Strategy rulebook", journal: "Research journal", market: "Market data & replay", strategy2: "Strategy 2 · Statistical mean reversion" };
  document.title = `${routeTitles[state.route]} — Range Lab`;
  const breadcrumb = $(".breadcrumb strong");
  if (breadcrumb) breadcrumb.textContent = routeTitles[state.route];
  drawCharts();
